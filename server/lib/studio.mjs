...
const TRADE_ACTION_RE = /平空|平多|平仓|开空|卖出开仓|开多|买入开仓|开仓/g;

function sideFromActionToken(token, text) {
  if (token === "平空") return "close-short";
  if (token === "平多") return "close-long";
  if (token === "平仓") return "close";
  if (token === "开空" || token === "卖出开仓") return "open-short";
  if (token === "开多" || token === "买入开仓") return "open-long";
  if (token === "开仓") return /空/.test(text) ? "open-short" : "open-long";
  return null;
}
...
export function parseTradeIntent(userText) {
  ...
  if (!text) return null;
  const isCancel = /撤单|取消委托|\bcancel\b/i.test(text) && !TRADE_ACTION_RE.test(text);
  TRADE_ACTION_RE.lastIndex = 0;
  ...
  const actionTokens = text.match(TRADE_ACTION_RE) || [];
  TRADE_ACTION_RE.lastIndex = 0;
  if (!actionTokens.length) return null;

  const contracts = [...text.matchAll(/\b([a-zA-Z]{1,4}\d{3,4})\b/g)].map((m) => m[1].toLowerCase());
  const uniqueContracts = [...new Set(contracts)];
  const lotsTokens = text.match(/[+-]?\d+(?:\.\d+)?\s*手/g) || [];
  if (actionTokens.length > 1 || uniqueContracts.length > 1 || lotsTokens.length > 1) {
    return { action: "reject", reason: "multi-intent", raw: text };
  }

  const side = sideFromActionToken(actionTokens[0], text) || "open-long";
  const questioned =
    /[？?]/.test(text) ||
    /(吗|么|呢)/.test(text) ||
    /可以吗|行吗|是否|要不要|能不能|风险是什么|什么风险/.test(text);
  const negated =
    /(不要|别|并非|不是要|请勿|拒绝).{0,24}(开|平|下单|委托)/.test(text) ||
    /(开|平|下单|委托).{0,40}(不要|别|并非|请勿|拒绝)/.test(text) ||
    /不要下单|别下单|先别下|不要成交/.test(text);
  if (questioned) return { action: "reject", reason: "question-intent", side, contract, lots: lotsRaw, raw: text };
      ...
      if (intent?.action === "reject") {
        const reasonText =
          intent.reason === "unknown-contract"
            ? "未识别合约代码"
            : intent.reason === "multi-intent"
              ? "一条消息里有多个交易动作或合约，请拆成单条明确指令"
              : intent.reason === "question-intent" || intent.reason === "negated-intent"
                ? "询问或否定不能当作下单意图"
                : "数量无效";
